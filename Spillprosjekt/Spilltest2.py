...
class player:
    ...
    def tegn(self):
        ...
        # Tegner kollisjonsboksen (rød rektangel)
        pygame.draw.rect(overflate, (255, 0, 0), self.kollisjon_rect, 2)
...
ildkuler = [ild(random.randint(0, vindux - 50), random.randint(0, vinduy - 50), 60, 60) for _ in range(3)]
...
while run:
    ...
    if alive:
        ...
        for ildkule in ildkuler:
            ...
            if spiller.rect.colliderect(ildkule.rect):
                alive = False
        
        # Legger til nye ildkuler etter en viss tid
        gjeldende_tid = pygame.time.get_ticks()
        if gjeldende_tid - sist_pluss >= pluss_intervall:
            ildkuler.append(ild(random.randint(0, vindux - 50), random.randint(0, vinduy - 50), 60, 60))
        ...
        if keys[pygame.K_r]:  # Restart med "R"
            alive = True
            ildkuler = [ild(random.randint(0, vindux - 50), random.randint(0, vinduy - 50), 60, 60) for _ in range(2)]
            spiller = player(300, 300, 50, 50)
